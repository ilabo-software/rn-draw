 ...
 */
const generateSVGPaths = (paths, simplifyOptions) => paths.map(i => ({
  ...i,
...
const Canvas = /*#__PURE__*/(0, _react.forwardRef)((_ref, ref) => {
  ...
  const panGesture = _reactNativeGestureHandler.Gesture.Pan().runOnJS(true).onChange(_ref2 => {
    let {
      x,
      y
    } = _ref2;
      ...
      case _types.DrawingTool.Eraser:
        setPaths(prevPaths => prevPaths.reduce((acc, p) => {
          const filteredDataPaths = p.data.reduce((acc2, data, index) => {
            const closeToPath = data.some(_ref3 => {
              let [x1, y1] = _ref3;
              return Math.abs(x1 - x) < p.thickness + eraserSize && Math.abs(y1 - y) < p.thickness + eraserSize;
            });
