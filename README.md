...
```sh
npm install @ilabo/rn-draw
# or
yarn add @ilabo/rn-draw
```
...
Supporting components, such as `CanvasControls`, `ColorPicker` and `BrushProperties` components, are available as a separate package, [`@ilabo/rn-draw-extras`](https://github.com/BenJeau/react-native-draw/tree/master/packages/react-native-draw-extras)
...
import { Canvas } from '@ilabo/rn-draw';
...
import { Canvas, CanvasRef } from '@ilabo/rn-draw';
...
### Example with `@ilabo/rn-draw-extras`

This uses the [`@ilabo/rn-draw-extras`](https://github.com/BenJeau/react-native-draw/tree/master/packages/react-native-draw-extras) npm package for the color picker and the bottom buttons/brush preview.

> As this package does not depend on `@ilabo/rn-draw-extras`, it is completely optional and you can build your own supporting UI, just like the previous example
